...
@section('content')
    ...
    <div class="container-fluid">
        <div class="mb-4">
            {{-- Title and Breadcrumb updated for editing --}}
            <h1 class="h3 text-gray-800">{{ $pageTitle ?? 'Edit Surat Pesanan' }}</h1>
            <x-breadcrumb :items="[
                ...
                <form action="{{ route('sp.update', $sp->id) }}" method="POST">
                    ...
                    <div class="row mb-3">
                        <div class="col-md-3 form-group">
                            ...
                            <input type="text" name="nomor_sp" class="form-control" value="{{ old('nomor_sp', $sp->nomor_sp) }}" required>
                        </div>
                        <div class="col-md-3 form-group">
                            ...
                            <select name="penyedia_id" class="form-control select2" required>
                                ...
                                @foreach ($penyedias as $p)
                                    ...
                                    <option value="{{ $p->id }}" {{ old('penyedia_id', $sp->penyedia_id) == $p->id ? 'selected' : '' }}>
                                        {{ $p->nama_penyedia }}
                                    </option>
                                @endforeach
                            </select>
                        ...
                        @if (auth()->user()->hasRole('admin'))
                            <div class="col-md-3 form-group">
                                ...
                                <select name="jenis_akun" id="jenis_akun" class="form-control @error('jenis_akun') is-invalid @enderror" required>
                                    <option value="">-- Pilih Jenis Akun --</option>
                                    {{-- 'selected' attribute added based on saved data --}}
                                    <option value="52" {{ old('jenis_akun', $sp->jenis_akun) == '52' ? 'selected' : '' }}>52 | Belanja Barang</option>
                                    <option value="53" {{ old('jenis_akun', $sp->jenis_akun) == '53' ? 'selected' : '' }}>53 | Belanja Modal</option>
                                </select>
                                @error('jenis_akun')
                        ...
                        @elseif($isRole52 || $isRole53)
                            <input type="hidden" name="jenis_akun" id="jenis_akun_hidden" value="{{ old('jenis_akun', $sp->jenis_akun) }}">
                        @endif
                        <div class="col-md-3 form-group">
                    ...
                    <div class="row mb-3">
                        <div class="col-md-3 form-group">
                            ...
                            <input type="date" name="tanggal" class="form-control" value="{{ old('tanggal', $sp->tanggal) }}" required>
                        </div>
                        <div class="col-md-3 form-group">
                            <label>Mulai Pekerjaan</label>
                            <input type="date" name="mulai_pekerjaan" id="mulai" class="form-control" value="{{ old('mulai_pekerjaan', $sp->mulai_pekerjaan) }}" required>
                        </div>
                        <div class="col-md-3 form-group">
                            <label>Masa (hari)</label>
                            <input type="number" name="masa" id="masa" class="form-control" value="{{ old('masa', $sp->masa) }}" required>
                        </div>
                        <div class="col-md-3 form-group">
                            <label>Akhir Pekerjaan</label>
                            <input type="text" id="akhir" class="form-control text-muted bg-light" readonly>
                            <input type="hidden" name="akhir_pekerjaan" id="akhir_pekerjaan_hidden" value="{{ old('akhir_pekerjaan', $sp->akhir_pekerjaan) }}">
                        </div>
                    </div>
                    <div class="row mb-3">
                        <div class="col-md-3 form-group">
                            <label>Total Pagu</label>
                            <input type="number" name="total_pagu" id="total_pagu" class="form-control text-muted bg-light" value="{{ old('total_pagu', $sp->total_pagu) }}" readonly>
                        </div>
                        <div class="col-md-3 form-group">
                            <label>Metode</label>
                            <select name="metode" class="form-control" required>
                                ...
                                @foreach ($metodes as $metode)
                                    <option value="{{ $metode }}" {{ old('metode', $sp->metode) == $metode ? 'selected' : '' }}>
                                        {{ $metode }}
                                    </option>
                                @endforeach
                            </select>
                        ...
                        <div id="dokumen-pemilihan-field" class="col-md-6 form-group" style="display: none;">
                            ...
                            <select name="dokumen_pemilihan_id" class="form-control">
                                ...
                                @foreach ($dokumenPemilihan as $dok)
                                    <option value="{{ $dok->id }}" {{ old('dokumen_pemilihan_id', $sp->dokumen_pemilihan_id) == $dok->id ? 'selected' : '' }}>
                    ...
                    <div class="row mb-3">
                        <div class="col-md-3 form-group">
                            <label>Akun</label>
                            <input type="text" name="akun" id="akun" class="form-control text-muted bg-light" value="{{ old('akun', $sp->akun) }}" readonly>
                        </div>
                    </div>
                    <div class="mt-4">
